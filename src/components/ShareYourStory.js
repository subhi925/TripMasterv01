...
function DetailsDialog({ item, onClose }) {
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if (e.key === "ArrowRight")
        setLb((s) => ({ open: true, i: (s.i + 1) % images.length }));
    };
          ...
          <section>
            ...
            <table className="sys-table">
              <tbody>
                <tr>
                  <th>Published</th>
                  <td>{fmt(item.shared_at || item.created_at)}</td>
                </tr>
                <tr>
                  <th>Duration</th>
...
export default function ShareYourStory() {
  ...
  const doClone = async () => {
    ...
    const s = clone.item;
    const id = getId(s);
    console.log("CLONE id=", id, "date=", s);
    if (!id) {
  ...
  const filtered = useMemo(() => {
    ...
    if (ratingEq !== "all")
      arr = arr.filter((x) => Number(x.rating) === Number(ratingEq));
    if (daysFilter !== "any") {
      arr = arr.filter((x) => {
    ...
    <div className="sys-wrapper">
      ...
      {!user && (
        <div className="sys-uid-banner">
          <span>
            Enter your <b>UID</b> (if not logged in)
          </span>
          <input
            className="sys-input"
            value={uidInput}
                ...
                <div className="sys-actions">
                  ...
                  {cloned ? (
                    ...
                    <button
                      type="button"
                      className={`sys-btn ${
                        cloned ? "sys-btn-success" : "sys-btn-primary"
                      }`}
